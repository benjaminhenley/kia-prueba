...
const ConcesionariosVenta = () => {
  ...
  const fetchDealersData = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://fusio.encender-dev.online/public/kia/concesionarios-list');
      ...
      const uniqueProvinces = [...new Set(ventaDealers.map(dealer => dealer.provincia))];
  ...
  const handleSearch = (province) => {
    if (!province || province === "todos") {
      ...
      const filtered = allDealers.filter((dealer) => {
        const normalizedDealerProvince = normalizeProvinceName(dealer.provincia);
