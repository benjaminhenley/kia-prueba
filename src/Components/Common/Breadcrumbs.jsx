...
const Breadcrumbs = () => {
  ...
  const needsConcesionariosPrefix =
    specialRoutes.routesWithConcesionariosPrefix.includes(currentPath);
  ...
  const isSegmentClickable = (segment, index) => {
    ...
    if (
      index === 0 &&
      specialRoutes.nonClickableFirstSegment.includes(segment)
    ) {
      return false;
    }
    ...
    <nav className="flex gap-2 items-center text-sm text-white font-medium flex-wrap">
      ...
      {isVehiclePage ? (
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <SeparatorIcon />
          <div className="pt-1 capitalize font-semibold">
            {vehicle.categoria}
          </div>
          <SeparatorIcon />
        ...
        <>
          ...
          {needsConcesionariosPrefix && (
            <>
              <SeparatorIcon />
              <span className="pt-1 font-semibold capitalize">
                Concesionarios
              </span>
            </>
          )}
            ...
            return (
              <Fragment key={routeTo}>
                ...
                {isLast || !shouldBeClickable ? (
                  ...
                  <Link to={routeTo}>
                    <div
                      className={`pt-1 cursor-pointer relative group font-semibold text-kia-polar-white hover:text-[#CDD0D2] transition-all duration-300${
                        !isCustom ? " capitalize" : ""
                      }`}>
                      {segmentTitle}
